...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(int[] array, int top, int last) {
        ...
        // "inorder" indicates whether the heap property is already satisfied.
        // If true, no further swaps are needed.
        boolean inorder = false;
        int root = top; // Start from the given root index.
        // Keep going while the current node has at least a left child (i.e. not a leaf)
        // and the heap property has not been restored yet.
        while (2 * root + 1 <= last && !inorder) {
            // Compute indices for left and right children.
            int left = 2 * root + 1;
            int right = 2 * root + 2;
            // Assume the smaller child is the left one first.
            int smallerChild = left;
            // If right child exists and is smaller than left child,
            // then choose the right child instead.
            if (right <= last && array[right] < array[left]) {
                smallerChild = right;
            }
            // If the parent is already smaller than or equal to the smaller child,
            // the heap property holds, so we are done.
            if (array[root] <= array[smallerChild]) {
                inorder = true;
            } else {
                // Otherwise, swap the parent with its smaller child
                // and continue sifting down from the child's position.
                exchangeEntries(array, root, smallerChild);
                root = smallerChild; // move down one level
            }
        }
    }
