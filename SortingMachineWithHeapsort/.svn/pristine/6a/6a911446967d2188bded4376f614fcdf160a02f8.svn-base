...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last, Comparator<T> order) {
        ...
        assert isHeap(array, 2 * top + 2, last, order)
                : "" + "Violation of: SUBTREE_IS_HEAP(array, 2 * top + 2, last,"
                        + " [relation computed by order.compare method])";
        // Get left nodes index
        int left = 2 * top + 1;
        // Judge whether there exists left node
        if (left > last) {
            return;
        }
        // Get right nodes index and set up smallest node index
        int right = 2 * top + 2;
        int smallest = left;
        if (right <= last && order.compare(array[right], array[left]) < 0) {
            smallest = right;
        }
        // If root node larger than smallest one, then recursively call siftDown
        // until root is the smallest one
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        // Define left and right and last index
        int left = 2 * top + 1;
        int right = 2 * top + 2;
        int last = array.length - 1;
        // Recursively heapify the left subtree if it exists.
        if (left <= last) {
            heapify(array, left, order);
        }
        // Recursively heapify the right subtree if it exists.
        if (right <= last) {
            heapify(array, right, order);
        }
        // Sift the current root element down to restore heap order.
